...
var myGameArea = {
    ...
    start : function() { // Definerer egenskaxper til canvasen/spillområdet
...
if(event.keyCode == 32) { //Spacebar, starter spillet på nytt, tømmer alle arrayene, reseter score
  myGameArea.stop()
  startGame();
  gameRunning = true;
  myObstacles = [];
...
function component(width, height, color, x, y, type, powerup) {
    ...
    this.gravitySpeed = 0;
    this.bounce = 0;
    this.update = function() { //Oppdaterer plasseringen til objektene
...
let light = true;
var lightInterval;
function randomLighting(){ // Definerer hvor ofte en powerup skal spawne
  let test2 = Math.random() < 0.8 ? (Math.floor((Math.random() * (6-4)+4)) * 1000) : 50;
...
myGameArea.canvas.onclick = (e) => { //Sjekker om x- og y-verdiene til musen er innenfor mute-knapper, og kjører mutefunksjonen hvis de er
  var pos = {
...
let variabel;
var myTiles = [  // Array som lagrer bildene for obstacles

];
var myTiles = [ // Annen array som lagrer bildene for obstacles
...
if(score > 2000) { // Obstacles spawner oftere når man kommer over 2000 poeng
  variabel = spawnRate
}

window.setTimeout(spawnObstacle, variabel)
}
function spawnObstacle() { // Funksjon som oppretter og spawner obstacles
...
myTiles.push([]);
for(let i = 0; i < tiles/20; i++) { // Oppretter bilder for obstacles og legger de oppå obstacles
...
function randomPowerup(){ // Definerer hvor ofte en powerup skal spawne
  let test2 = (Math.random() * (15-10)+10) * 1000
  console.log("Spawnrate "+spawnRate);
  window.setTimeout(spawnPowerup, test2)
}

function spawnPowerup() { // Spawner powerups
  var powerup = parseInt(Math.floor(Math.random()*(myPowerUps[0].length-1)));
...
var textX = 200;
function welcomeText() { // Tekst som kommer når spillet starter
  y = 50;
  textX -= 2;
  text.font = "30px Arial";
  text.fillStyle = "#562a73";
  text.fillText("Klarer du å hjelpe Stein med å få det gyldne lønnspålegget?",textX,125);
}
    ...
    function saveInput(evt){
      if(evt.keyCode == 32 || pressed == true) {
        ...
        if (input.value != "") {
          ...
          // inputName.removeChild(input);
          // inputName.parentNode.removeChild(btn);
          // input.remove();
          // btn.remove();
